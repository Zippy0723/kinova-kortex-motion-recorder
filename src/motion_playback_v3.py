...
class MotionPlayback:
    ...
    def playback_cartesian_pos_list(self,cartesian_pos_list):
        ...
        global timer
        timer = threading.Timer(30, self.timeout_abort)
        timer.start()
    ...
    def main(self):
        ...
        if success:
            ...
            while True:
                user_token = input("Please Enter the Filename of the motion you'd like played:")
                try:
                    with open(user_token,'r') as file:
                        action_queue = json.load(file)
                        print("Data loaded succesfully. Playing back motion....")
                    ...
                    print("File not found! Please make sure you have a valid motion .json file")

        if not success:
            rospy.logerr("The program has encountered an error. Please check the readme")
        else:
            print("Succesfully played back recorded Actions.")
